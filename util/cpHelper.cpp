...
cpHelper* cpHelper::setHandlerPath(const std::wstring& path)
{
	d->handlerPath = path;
	return this;
}

cpHelper* cpHelper::setDumpPath(const std::string& path)
{
	d->dumpPath = path;
	return this;
}

std::string cpHelper::uploadUrl() const
{
    return d->uploadUrl;
}


std::string cpHelper::dumpPath() const
{
	return d->dumpPath;
}

cpHelper* cpHelper::setUploadUrl(const std::string &urlstr)
{
...
cpHelper* cpHelper::setDBPath(const std::wstring& path)
{
	this->d->dbPath = path;
	return this;
}
...
void cpHelper::crash_fun()
{    
    const auto p = static_cast<int*>(nullptr);
    *p = 1;
}

int cpHelper::func(bool isCrash)
{
	if (isCrash)
	{
